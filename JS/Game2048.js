...
(function(window, document, $) {
  ...
  function Game2048(opt) {
    //自动生成一个棋盘格局，提供一个对象传参
      ...
      * opt.len:    棋盘为len*len规格
      * opt.size:   棋盘每个格子的大小
      * opt.margin:   棋盘中每个格子之间的外边距大小
      * */
    let prefix = opt.prefix, len = opt.len, size = opt.size, margin = opt.margin;
    //初始分数
    let score = 0;
    //获胜数值设定
    let winNum = 2048;
  ...
  Board.prototype = {
    ...
    init: function() {
      let arr = []
      for ( let x = 0, len = this.len; x < len; ++x) {
        arr[x] = [];
        for (let y = 0; y < len; ++y) {
            arr[x][y] = 0;
          }
    ...
    generate: function() {
      let empty = [];
      for (let x = 0, arr = this.arr, len = arr.length; x < len; ++x) {
        for (let y = 0; y < len; ++y) {
          if (arr[x][y] === 0) {
            //  记录空的单元格
            empty.push({x: x, y: y});
    ...
    moveLeft: function() {
      let canMove = false;
      // 从上到下，从左到右
      for (let x = 0, len = this.arr.length; x < len; ++x) {
        for (let y = 0, arr = this.arr[x]; y < len; ++y) {
          // 从 y + 1 位置开始，向右查找
          for (let next = y + 1; next < len; ++next) {
    ...
    moveRight: function() {
      let moved = false;
       for (let x = 0, len = this.arr.length; x < len; ++x) {
        for (let y = len - 1, arr = this.arr[x]; y >= 0; --y) {
          for (let prev = y - 1; prev >= 0; --prev) {
            if (arr[prev] === 0) {
              continue;
            }
            if (arr[y] === 0) {
    ...
    moveUp: function() {
      let canMove = false;
      for (let arr = this.arr, len = arr.length, y = 0; y < len; ++y) {
        for (let x = 0; x < len; ++x) {
          for (let next = x + 1; next < len; ++next) {
            if (arr[next][y] === 0) {
              continue;
            }
            if (arr[x][y] === 0) {
    ...
    canMove: function() {
      for (let x = 0, arr = this.arr, len = arr.length; x < len; ++x) {
        for (let y = 0; y < len; ++y) {
          ...
          let curr = arr[x][y], right = arr[x][y + 1];
          let down = arr[x + 1] ? arr[x + 1][y] : null;
  ...
  function View(prefix, len, size, margin) {
    ...
    let containerSize = len * size + margin * (len + 1);
  ...
  View.prototype = {
    ...
    init: function() {
      for (let x = 0, len = this.len; x < len; ++x) {
        for (let y = 0; y < len; ++y) {
          let $cell = $(`<div class="${this.prefix}-cell"></div>`);
          $cell.css({
              width: this.size + 'px',
              height: this.size + 'px',
              top: this.getPos(x),
    ...
    addNum: function(x, y, num) {
      let $num = $(`<div class="${this.prefix}-num ${this.prefix}-num-${num}"></div>`);
      //设置格子中数字居中
      $num.text(num).css({
    ...
    move: function(from, to) {
      let fromIndex = from.x + '-' + from.y, toIndex = to.x + '-' + to.y;
      let clean = this.nums[toIndex];
      this.nums[toIndex] = this.nums[fromIndex];
      delete this.nums[fromIndex];
      let prefix = this.prefix + '-num-';
      let pos = {top: this.getPos(to.x), left: this.getPos(to.y)};
